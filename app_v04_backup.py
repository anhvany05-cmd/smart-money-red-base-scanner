...
def main():
    ...
    st.title("Smart Money Red Base Scanner – MVP V0.4")
    st.caption("Quét cổ phiếu có dấu hiệu tay to gom hàng, ưu tiên mua đỏ trong nền, không mua xanh/đu break.")
    ...
    st.sidebar.header("1) Dữ liệu")
    use_demo = st.sidebar.toggle("Dùng dữ liệu demo", value=True)
    price_file = st.sidebar.file_uploader("Upload prices.csv/xlsx", type=["csv", "xlsx", "xls"], disabled=use_demo)
    index_file = st.sidebar.file_uploader("Upload vnindex.csv/xlsx", type=["csv", "xlsx", "xls"], disabled=use_demo)
    fundamental_file = st.sidebar.file_uploader("Upload fundamentals.csv/xlsx tùy chọn", type=["csv", "xlsx", "xls"], disabled=use_demo)
    ...
    try:
        if use_demo:
            ...
            st.info("Đang dùng dữ liệu demo để minh họa. Khi dùng thật, hãy tắt demo và upload file giá/VNIndex của bạn.")
        else:
            if price_file is None or index_file is None:
                st.warning("Vui lòng upload tối thiểu 2 file: prices và VNIndex.")
                st.stop()
